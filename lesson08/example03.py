...
class NotIntegerListException(Exception):
...
while True:
    try:
        for item in input("Enter numbers separated by space >>> ").split():
            if item == "stop":
                exit()
            if item.isdigit() is not True:
                raise NotIntegerListException("Not only numbers")
    except NotIntegerListException as message:
        print(message)
